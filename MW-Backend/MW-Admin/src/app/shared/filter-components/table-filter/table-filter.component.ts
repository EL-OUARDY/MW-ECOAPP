...
import { MatDialog, MatDatepickerInput, DateAdapter } from "@angular/material";
...
export class TableFilterComponent {
  ...
  doFilter() {
    ...
    if (this.searchQuery) this.queryObj.Search = this.searchQuery.toLowerCase();
    else this.queryObj.Search = undefined;

    if (this.minDate && this.maxDate) {
      // string
      this.queryObj.MinDate = DateTime.fromJSDate(this.minDate).toFormat(
        "yyyy-MM-dd"
      );
      this.queryObj.MaxDate = DateTime.fromJSDate(this.maxDate).toFormat(
        "yyyy-MM-dd"
      );
    }
  ...
  minDateInputChange() {
    ...
    if (
      DateTime.fromJSDate(this.minDate).valueOf() >
      DateTime.fromJSDate(this.maxDate).valueOf()
    )
